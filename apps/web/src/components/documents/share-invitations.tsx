...
export function ShareInvitations() {
  ...
  const invitations = trpc.documents.shareInvitations.useQuery(undefined, {
  ...
  const accept = trpc.documents.acceptShare.useMutation({
    onSuccess: async () => {
      ...
      setProjectId('')
      await Promise.all([
        utils.documents.shareInvitations.invalidate(),
  ...
  const decline = trpc.documents.declineShare.useMutation({
    onSuccess: () => {
      ...
      void utils.documents.shareInvitations.invalidate()
    },
      ...
      <DialogTrigger
        render={
          <Button
            ...
            size="sm"
            className="relative"
            aria-label="Document invitations"
          >
            <Bell data-icon="inline-start" />
            Invitations
            {count ? <Badge className="ml-1 min-w-5 justify-center px-1">{count}</Badge> : null}
          </Button>
        }
      />
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-serif">Document invitations</DialogTitle>
          <DialogDescription>
            Accepting adds the document to one of your projects. Its content stays shared; its path
            is yours.
          </DialogDescription>
        </DialogHeader>
        {!selected ? (
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setProjectId('')
                          setPath(invitation.documentTitle)
                        }}
                      >
          ...
          <>
            ...
            <Field className="mt-3">
              <FieldLabel htmlFor="share-mount-path">File path in that project</FieldLabel>
              <Input
                id="share-mount-path"
                value={path}
            ...
            <DialogFooter className="mt-5">
              ...
              <Button
                disabled={!projectId || !path.trim() || accept.isPending}
                onClick={() =>
                  accept.mutate({ invitationId: selected.id, projectId, path: path.trim() })
                }
              >
                {accept.isPending ? 'Adding…' : 'Add to project'}
              </Button>
            </DialogFooter>
          </>
        )}
