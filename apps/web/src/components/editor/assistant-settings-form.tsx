...
export function AssistantSettingsForm({
  ...
  const output = (): AssistantPreferenceOverrides =>
    mode === 'inherit' ? {} : { defaultMode: mode }
    ...
    <div className="space-y-4">
      <Field>
        ...
        <FieldContent>
          <Select
            value={mode}
            onValueChange={(value) => setMode(value as AssistantMode | 'inherit')}
          >
            <SelectTrigger id="assistant-default-mode">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {allowInherit && (
                <SelectItem value="inherit">Inherit ({labels[resolved.defaultMode]})</SelectItem>
              )}
      ...
      <p className="text-muted-foreground text-xs">
        This is a default only. Existing project conversations keep their selected mode.
      </p>
      <div className="flex justify-end gap-2">
        <Button
          ...
          onClick={() => setMode(direct.defaultMode ?? 'inherit')}
        >
          Reset
        </Button>
        <Button type="button" disabled={!dirty || isSaving} onClick={() => onSave(output())}>
          {isSaving ? 'Saving…' : 'Save assistant default'}
        </Button>
      </div>
    </div>
  )
