...
import type { DocumentCollaborator, DocumentShareInvitation } from '../ports/documentCollaborators.port.js'

export type DocumentEffectiveRole = 'owner' | DocumentAccessRole | null
export type AcceptInvitationResult =
  | { status: 'accepted'; invitation: DocumentShareInvitation }
  | { status: 'not_found' | 'inactive' | 'destination_not_found' | 'destination_not_owned' | 'path_conflict' | 'already_mounted' }
...
export interface DocumentSharingService {
  ...
  invite(input: { documentId: string; recipientUserId: string; role: DocumentAccessRole; invitedByUserId: string }): Promise<DocumentShareInvitation | null>
  acceptInvitation(input: { invitationId: string; userId: string; projectId: string; path: string }): Promise<AcceptInvitationResult>
  ...
  changeRole(input: { documentId: string; userId: string; role: DocumentAccessRole; actingUserId: string }): Promise<boolean>
  revokeAccess(input: { documentId: string; userId: string; actingUserId: string }): Promise<boolean>
  leave(documentId: string, userId: string): Promise<boolean>
  transferOwnership(input: { documentId: string; recipientUserId: string; recipientProjectId: string; actingUserId: string }): Promise<Document | null>
}
...
export function createDocumentSharingService(
  ...
  return {
    ...
    async listMountedProjectIds(documentId) {
      return (await repos.projectDocuments.listByDocument(documentId)).map((mount) => mount.projectId)
    },
    ...
    async invite(input) {
      ...
      const existing = await repos.documentCollaborators.find(input.documentId, input.recipientUserId)
      if (existing) return null
      const now = Date.now()
      const invitation: DocumentShareInvitation = {
        id: nanoid(), documentId: input.documentId, recipientUserId: input.recipientUserId,
        role: input.role, invitedByUserId: input.invitedByUserId, createdAt: now,
    ...
    async acceptInvitation(input) {
      ...
      if (invitation.acceptedAt || invitation.declinedAt || invitation.revokedAt) return { status: 'inactive' }
      ...
      if (existing.some((mount) => mount.documentId === invitation.documentId)) return { status: 'already_mounted' }
      if (existing.some((mount) => mount.path === input.path && mount.documentId !== invitation.documentId)) return { status: 'path_conflict' }
      const now = Date.now()
      await transaction.run(async () => {
        await repos.documentCollaborators.upsert({
          documentId: invitation.documentId, userId: input.userId, role: invitation.role,
          grantedByUserId: invitation.invitedByUserId, grantSource: 'invite', createdAt: now, updatedAt: now,
        })
        await repos.projectDocuments.insert({ projectId: input.projectId, documentId: invitation.documentId, path: input.path, addedByUserId: input.userId, addedAt: now, updatedAt: now })
    ...
    async declineInvitation(invitationId, userId) {
      ...
      if (!invitation || invitation.recipientUserId !== userId || invitation.acceptedAt || invitation.declinedAt || invitation.revokedAt) return null
      const now = Date.now()
    ...
    async revokeInvitation(invitationId, actingUserId) {
      const invitation = await repos.documentCollaborators.findInvitation(invitationId)
      if (!invitation || invitation.acceptedAt || invitation.declinedAt || invitation.revokedAt) return false
    ...
    async changeRole(input) {
      ...
      await repos.documentCollaborators.upsert({ ...collaborator, role: input.role, updatedAt: Date.now() })
      return true
    },
    ...
    async leave(documentId, userId) {
      const document = await repos.documents.findById(documentId)
      const homeProject = document && await repos.projects.findById(document.homeProjectId)
      if (!document || homeProject?.ownerUserId === userId || !await repos.documentCollaborators.find(documentId, userId)) return false
      await transaction.run(async () => {
        const mounts = await repos.projectDocuments.listByDocument(documentId)
        for (const mount of mounts) {
          const project = await repos.projects.findById(mount.projectId)
          if (project?.ownerUserId === userId) await repos.projectDocuments.delete(mount.projectId, documentId)
        }
    ...
    async transferOwnership(input) {
      ...
      if (!document || !recipientProject || recipientProject.ownerUserId !== input.recipientUserId || document.homeProjectId === input.recipientProjectId) return null
      ...
      await transaction.run(async () => {
        ...
        if (!mounts.some((mount) => mount.documentId === input.documentId)) {
          const currentHomeMount = (await repos.projectDocuments.listByDocument(input.documentId)).find(
            (mount) => mount.projectId === document.homeProjectId
          )
          await repos.projectDocuments.insert({ projectId: input.recipientProjectId, documentId: input.documentId, path: currentHomeMount?.path ?? document.title, addedByUserId: input.recipientUserId, addedAt: now, updatedAt: now })
        }
        await repos.documents.update(input.documentId, { homeProjectId: input.recipientProjectId, updatedAt: now })
        ...
        if (previousHome) await repos.documentCollaborators.upsert({ documentId: input.documentId, userId: previousHome.ownerUserId, role: 'editor', grantedByUserId: input.recipientUserId, grantSource: 'ownership_transfer', createdAt: now, updatedAt: now })
      })
