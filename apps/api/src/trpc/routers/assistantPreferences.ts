...
export const assistantPreferencesRouter = router({
  getUser: protectedProcedure.query(({ ctx }) =>
    ctx.services.assistantPreferences.snapshot(ctx.user.id)
  ),
  updateUser: protectedProcedure
    .input(z.object({ overrides: assistantPreferenceOverridesSchema }))
  ...
  updateProject: protectedProcedure
    .input(z.object({ projectId: id, overrides: assistantPreferenceOverridesSchema }))
    .mutation(async ({ ctx, input }) => {
      await assertProjectAccess(ctx, input.projectId)
